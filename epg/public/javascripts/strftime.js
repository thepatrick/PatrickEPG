...
Date.prototype.strftime =
	...
	Date.prototype.UTCstrftime_f = {
			A: function (d) { return d.weekdays[d.getUTCDay()] },
			a: function (d) { return d.weekdays[d.getUTCDay()].substring(0,3) },
			B: function (d) { return d.months[d.getUTCMonth()] },
			b: function (d) { return d.months[d.getUTCMonth()].substring(0,3) },
			C: function (d) { return Math.floor(d.getUTCFullYear()/100); },
			c: function (d) { return d.toString() },
			D: function (d) {
					return d.strftime_f.m(d) + '/' +
						d.strftime_f.d(d) + '/' + d.strftime_f.y(d);
			...
			H: function (d) { return d.getUTCHours().pad(2,'0') },
			I: function (d) { return ((d.getUTCHours() % 12 || 12).pad(2)) },
			j: function (d) {
					var t = d.getUTCDate();
			...
			k: function (d) { return d.getUTCHours().pad(2,' ') },
			l: function (d) { return ((d.getUTCHours() % 12 || 12).pad(2,' ')) },
			M: function (d) { return d.getUTCMinutes().pad(2,'0') },
			m: function (d) { return (d.getUTCMonth()+1).pad(2,'0') },
			n: function (d) { return "\n" },
			p: function (d) { return (d.getUTCHours() > 11) ? 'PM' : 'AM' },
			R: function (d) { return d.UTCstrftime_f.H(d) + ':' + d.UTCstrftime_f.M(d) },
			r: function (d) {
					return d.UTCstrftime_f.I(d) + ':' + d.UTCstrftime_f.M(d) + ':' +
						d.UTCstrftime_f.S(d) + ' ' + d.UTCstrftime_f.p(d);
				},
			S: function (d) { return d.getUTCSeconds().pad(2,'0') },
			s: function (d) { return Math.floor(d.getUTCTime()/1000) },
			T: function (d) {
					return d.UTCstrftime_f.H(d) + ':' + d.UTCstrftime_f.M(d) + ':' +
						d.UTCstrftime_f.S(d);
				},
			t: function (d) { return "\t" },
	/*		U: function (d) { return false }, */
			u: function (d) { return(d.getUTCDay() || 7) },
	/*		V: function (d) { return false }, */
			v: function (d) {
					return d.UTCstrftime_f.e(d) + '-' + d.UTCstrftime_f.b(d) + '-' +
						d.UTCstrftime_f.Y(d);
				},
	/*		W: function (d) { return false }, */
			...
			y: function (d) { return (d.getUTCYear() % 100).pad(2) },
	//		Z: function (d) { return d.toString().match(/\((.+)\)$/)[1]; },
	//		z: function (d) { return d.getTimezoneOffset() }, // wrong
	//		z: function (d) { return d.toString().match(/\sGMT([+-]\d+)/)[1]; },
			'%': function (d) { return '%' }
	...
	Date.prototype.UTCstrftime_f['+'] = Date.prototype.UTCstrftime_f.c;
	Date.prototype.UTCstrftime_f.h = Date.prototype.UTCstrftime_f.b;

	Date.prototype.UTCstrftime =
		function (fmt) {
			var r = '';
			var n = 0;
			while(n < fmt.length) {
				...
				if (c == '%') {
					c = fmt.substring(++n, n+1);
					r += (this.UTCstrftime_f[c]) ? this.UTCstrftime_f[c](this) : c;
				} else r += c;
				++n;
			}
			return r;
